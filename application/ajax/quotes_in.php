...
function in_data(){
	...
	$sql="CREATE TEMPORARY TABLE `temp_data_in` SELECT quotes_in.id_quotes_in AS id_quotes_in, quotes_in.id_user AS id_user, quotes_in.name AS name,
...
//поиск по стране
	if($_POST['p_target']=='country'){
		$sql="SELECT DISTINCT id_quotes_in, country FROM temp_data_in";
		if(isset($keys_in)){
			$sql.=" WHERE ";
...
//поиск по городу
	if($_POST['p_target']=='city'){
		$sql="SELECT id_quotes_in, city FROM temp_data_in";
		if(isset($keys_in)){
			$sql.=" WHERE ";
			$counter_keys=count($keys_in);
			$keys=array_keys($keys_in);
...
//поиск по группе
	if($_POST['p_target']=='farm_group'){
		$sql="SELECT DISTINCT id_quotes_in, groupsmedicines FROM temp_data_in";
		if(isset($keys_in)){
			$sql.=" WHERE ";
			$counter_keys=count($keys_in);
			$keys=array_keys($keys_in);
...
//поиск по форме
	if($_POST['p_target']=='farm_form'){
		$sql="SELECT DISTINCT id_quotes_in, form FROM temp_data_in";
		if(isset($keys_in)){
			$sql.=" WHERE ";
			$counter_keys=count($keys_in);
			$keys=array_keys($keys_in);
...
//поиск по производителю
	if($_POST['p_target']=='manufacturer'){
		$sql="SELECT DISTINCT id_quotes_in, manufacturer FROM temp_data_in";
		if(isset($keys_in)){
			$sql.=" WHERE ";
			$counter_keys=count($keys_in);
			$keys=array_keys($keys_in);
