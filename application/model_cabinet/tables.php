...
{
	public function get_tables(){					//страница работы с таблицами
	if(isset($_SESSION['data'])){
		if(isset($_POST['back'])){
			$this->exit_error();
		}
		$link=db_mysql_connect();
		$sql="SELECT * FROM manadge_menu WHERE level >= ".$_SESSION['level']." and adres='/tables'";
		...
		if($counters>0){
			...
			$data.='<td valign="top"><h1 class="h1 med">Управление таблицами в базе данных</h1><br><br>';
			$massive=$this->take_url();
			$func_name=$massive[0].'tables';
	...
	public function get_creat(){					//создание заявок
		if(isset($_SESSION['data'])){
			if(isset($_POST['back'])){
				$this->exit_error();
			}
...
//режим редактирования
			if(isset($_SESSION['target'])){
				...
				$sql="SELECT * FROM $table WHERE id_user='".$_SESSION['data']."' and id_$table='$id'";
				$result = mysql_query($sql);	
				if($result==false){
					$data=$this->error_sql();
					return $data;
				...
				if($counter_id>0){
					$sql="SELECT * FROM $sity WHERE id_$table='$id'";
...
//редактирование заявки
			if(isset($_POST['paste_quotes'])){
				...
				if($counter>0){
					...
					$sql="UPDATE $table SET name='$med_name', count='".$_POST['quote_count']."', form='".$_POST['radio_form']."', manufacturer='".$_POST['radio_manufacture']."', photo_link='".$_POST['radio_photo']."', mail='$mail', tel='$tel' WHERE id_$table='$id'";
					$result = mysql_query($sql);	
					if($result==false){
						$data=$this->error_sql();
						return $data;
					...
					$i=0;$n=0;
					$sql="DELETE FROM $sity WHERE id_$table='$id'";
					$result = mysql_query($sql);	
					if($result==false){
						$data=$this->error_sql();
						return $data;
					...
					while($i<$counter){
						if(isset($_POST['checked'.$n])){
							$sql="INSERT INTO $sity(id_$table, id_sity) VALUES('$id','".$_POST['checked'.$n]."')";
							$result = mysql_query($sql);	
							if($result==false){
								$data=$this->error_sql();
								return $data;
...
//создание заявки
			if(isset($_POST['creat_quotes'])){
				...
				if($counter>0 and !empty($med_name) and !empty($_POST['radio_photo']) and !empty($_POST['quote_count']) and !empty($_POST['radio_form']) and !empty($_POST['radio_manufacture'])){
					...
					$sql="INSERT INTO $tables(id_user, name, count, form, manufacturer, photo_link, status, mail, tel) VALUES('".$_SESSION['data']."','$med_name','".$_POST['quote_count']."','".$_POST['radio_form']."','".$_POST['radio_manufacture']."','".$_POST['radio_photo']."','1','$mail','$tel')";
					$result = mysql_query($sql);	
					if($result==false){
					...
					while($i<$counter){
						if(isset($_POST['checked'.$n])){
							$sql="INSERT INTO $tables_city(id_quotes_in, id_sity) VALUES('$id_quota','".$_POST['checked'.$n]."')";
							$result = mysql_query($sql);	
							if($result==false){
								$data['messadge']=$this->error_sql();
							}
...
//формирование левого меню
			...
			if(isset($_POST['country']) and !empty($_POST['country'])){
				...
				$sql="SELECT id_links_25, city FROM links_25 WHERE country='$preselect' ORDER BY city";
				$result = mysql_query($sql);	
				if($result==false){
					$data['messadge']=$this->error_sql();
				}
...
//меню ввода параметров			
			if(isset($_POST['medical_name']) and  !empty($_POST['medical_name'])){
				$sql="SELECT DISTINCT form_type FROM medication WHERE names='$med_name'";
				$result = mysql_query($sql);	
				if($result==false){
					$data['messadge']=$this->error_sql();
				}
				...
				$sql="SELECT DISTINCT manufacturer FROM medication WHERE names='$med_name'";
				$result = mysql_query($sql);	
				if($result==false){
					$data['messadge']=$this->error_sql();
				}
				...
				$sql="SELECT DISTINCT photo_link FROM medication WHERE names='$med_name'";
				$result = mysql_query($sql);	
				if($result==false){
					$data['messadge']=$this->error_sql();
				}
